...
@section('content')
    <div class="container mx-auto p-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="p-4 bg-white shadow rounded">My Diagnoses: <strong>{{ $counts['diagnoses'] ?? 0 }}</strong></div>
            <div class="p-4 bg-white shadow rounded">Patients: <strong>{{ $counts['patients'] ?? 0 }}</strong></div>
            <div class="p-4 bg-white shadow rounded">Recent Images: <strong>{{ $counts['images'] ?? 0 }}</strong></div>
        </div>
